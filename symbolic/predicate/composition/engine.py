...
class CompositionEngine():
    ...
    def _calculate_co_occurence_(self,
        ...
        all_instances:          List[Dict[str, Any]] =  candidate.positive_evidence + \
        ...
        complete_pattern_count: int =                   sum(
                                                            1 
                                                            for instance
                                                            in all_instances
                                                            if len(instance.get('matched_predicates', [])) \
                                                                >= len(candidate.pattern.component_patterns)
                                                        )
    ...
    def _calculate_distinctiveness_(self,
        ...
        """        
        return  (
                    candidate.pattern_complexity + \
                    candidate.binding_diversity  + \
                    (max(0.0, candidate.confidence - 0.5) * 2.0)
                ) / 3.0
    ...
    def _evaluate_candidates_(self) -> None:
        ...
        for candidate in self._candidates_.values():
            ...
            candidate.update_advanced_metrics(
                co_occurrence =     self._calculate_co_occurence_(
                                        candidate = candidate
                                    ),
                distinctiveness =   self._calculate_distinctiveness_(
                                        candidate = candidate
                                    )
            )
    ...
    def _promote_candidates_(self) -> None:
        ...
        ready_candidates:   List[Tuple[str, Candidate]] =   [
                                                                ...
                                                                if candidate.meets_promotion_criteria
                                                            ]
